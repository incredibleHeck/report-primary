...
const AuditManager = {
  process: function () {
    ...
    // 1. FRESH SLATE RESET
    range.setFontColor("#ffffff"); // Reset to White (or default)
    ...
    if (result.success && result.issues === 0) {
      ...
      SpreadsheetApp.getActiveSpreadsheet().toast(
        `⚠️ Found ${result.issues} issues. Hover over RED cells.`,
        "Audit Done",
        -1,
      );
    }
  ...
  processRange: function (range) {
    ...
    if (!classlistSheet)
      throw new Error(
        `❌ Missing Classlist Sheet: "${Config.CLASSLIST_SHEET_NAME}"`,
      );
    ...
    for (let r = 0; r < subjectData.length; r++) {
      ...
      const gender =
        genderRaw && String(genderRaw).trim().toUpperCase().startsWith("F")
          ? "Female"
          : "Male";
    ...
    try {
      ...
      if (Array.isArray(results)) {
        results.forEach((res) => {
          if (res && res.id) resultMap[res.id] = res;
        });
      }
      ...
      batchRequest.forEach((item) => {
        ...
        if (
          result &&
          (result.hasError === true || String(result.hasError) === "true")
        ) {
          issuesFound++;
          fontColors[item.rowIndex][item.colIndex] = "#FF0000";
          const type = result.errorType || "ISSUE";
          notes[item.rowIndex][item.colIndex] =
            `🚩 ${type}:\n${result.feedback}`;
        }
      });
